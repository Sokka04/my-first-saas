...
export function SubscriptionAccountForm() {
  ...
  const [botChallengeId, setBotChallengeId] = useState("");
  const [botChallengeQuestion, setBotChallengeQuestion] = useState("");
  const [botChallengeAnswer, setBotChallengeAnswer] = useState("");
  ...
  useEffect(() => {
    ...
    async function loadChallenge() {
      try {
        const response = await fetch("/api/subscription/challenge", { method: "GET" });
        const data = (await response.json()) as {
          challengeId?: string;
          question?: string;
        };

        if (!response.ok || !data.challengeId || !data.question) {
          throw new Error("Challenge indisponible");
        }

        if (!cancelled) {
          setBotChallengeId(data.challengeId);
          setBotChallengeQuestion(data.question);
          setFormStartedAt(Date.now());
        }
      } catch {
        if (!cancelled) {
          setError("Impossible de charger la verification anti-bot. Recharge la page.");
        }
      }
    }
  ...
  function validateAccount() {
    ...
    if (!botChallengeId || !botChallengeQuestion) {
      return "Verification anti-bot indisponible. Recharge la page.";
    }
    if (!botChallengeAnswer.trim()) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.set("challengeId", botChallengeId);
      formData.set("challengeAnswer", botChallengeAnswer);
  ...
  return (
    <section className="border-border bg-card rounded-2xl border p-6 sm:p-8">
      ...
      >
        ...
        <div className="space-y-2">
          <Label htmlFor="botChallenge">Verification anti-bot</Label>
          <div className="flex gap-2">
            <Input
              id="botChallengeQuestion"
              className="h-11"
              value={botChallengeQuestion || "Chargement..."}
              readOnly
            />
            <Input
              id="botChallenge"
              className="h-11"
              placeholder="Reponse"
              value={botChallengeAnswer}
              onChange={(event) => setBotChallengeAnswer(event.target.value)}
            />
          </div>
          <p className="text-muted-foreground text-xs">
            Cette verification reduit les inscriptions automatisees.
          </p>
        </div>
        ...
        {showTurnstile ? (
          <div className="space-y-2">
            <Label>Verification supplementaire</Label>
            {turnstileSiteKey ? (
              <>
                <Script
              ...
              <p className="text-destructive text-xs">
                CAPTCHA non configure: ajoute NEXT_PUBLIC_TURNSTILE_SITE_KEY.
              </p>
            )}
          </div>
        ) : null}
