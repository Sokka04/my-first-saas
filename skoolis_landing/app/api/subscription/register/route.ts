...
function parsePayload(formData: FormData): ParsedPayload {
  return {
    ...
    challengeId: sanitizePlainText(String(formData.get("challengeId") ?? "")),
    challengeAnswer: sanitizePlainText(String(formData.get("challengeAnswer") ?? "")),
    ...
    turnstileToken: sanitizePlainText(String(formData.get("cf-turnstile-response") ?? "")),
  };
}
...
export async function POST(request: Request) {
  ...
  const challenge = validateMathChallenge(ip, payload.challengeId, payload.challengeAnswer);
  if (!challenge.ok) {
    return NextResponse.json({ ok: false, error: challenge.reason }, { status: 400 });
  }
  ...
  if (behavior.isSuspicious) {
    const captchaCheck = await verifyTurnstileToken(payload.turnstileToken, ip);
    if (!captchaCheck.ok) {
      return NextResponse.json(
        {
          ok: false,
          requiresCaptcha: true,
          error: captchaCheck.reason ?? "Verification CAPTCHA requise.",
        },
        { status: 403 }
      );
    }
...
async function verifyTurnstileToken(token: string, ip: string) {
  if (!token) {
    return { ok: false, reason: "Verification CAPTCHA manquante." };
  }
  ...
  if (!secret) {
    return {
      ok: false,
      reason: "CAPTCHA indisponible: configure TURNSTILE_SECRET_KEY.",
    };
  }
  ...
  if (!response.ok || !data.success) {
    return { ok: false, reason: "CAPTCHA invalide. Reessaie." };
  }
